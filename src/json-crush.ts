...
function crushValue(v: Json, opts: Required<CompressOptions>, depth: number): Json {
  ...
  if (Array.isArray(v)) return v.map((e) => crushValue(e, opts, depth + 1));
...
function crushObjectArray(arr: Record<string, unknown>[], opts: Required<CompressOptions>, depth: number): Json[] {
  const keep = opts.headItems + opts.tailItems;
  if (arr.length <= keep + 1) return arr.map((e) => crushValue(e, opts, depth + 1));
  ...
  for (const row of elidedRows) for (const k of Object.keys(row)) keys.add(k);
