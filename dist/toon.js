...
function classifyColumns(rows, keys) {
    const consts = {};
    const dicts = {};
    const cols = [];
    for (const k of keys) {
...
export function encodeColumnar(rows) {
    if (!isObjectArray(rows))
        return null;
    ...
    const dataRows = rows.map((r) => csvRow(cols.map((k) => {
        ...
        const dict = dicts[k];
        if (dict)
            return String(dict.findIndex((v) => JSON.stringify(v) === JSON.stringify(r[k])));
...
export function decodeColumnar(text) {
    const lines = text.split("\n");
    if (!lines[0]?.startsWith(`${HEADER_COL} `))
        return null;
    const n = parseInt(lines[0].slice(HEADER_COL.length + 1), 10);
    if (!Number.isFinite(n) || lines.length < 2)
        return null;
    let meta;
    try {
...
export function decodeTable(text) {
    if (text.startsWith(`${HEADER_COL} `))
        return decodeColumnar(text);
    const lines = text.split("\n");
    ...
    if (!Number.isFinite(n) || lines.length < 2)
        return null;
    const keys = parseCsv(lines[1]).map((c) => decodeCell(c));
    const rows = [];
    for (let i = 0; i < n; i++) {
        const line = lines[2 + i];
        if (line === undefined)
        ...
        const fields = parseCsv(line);
        const obj = {};
        keys.forEach((k, j) => { if (fields[j] !== undefined && fields[j] !== "")
            obj[k] = decodeCell(fields[j]); });
        rows.push(obj);
    }
    return rows;
}
 ...
 */
export function toonCompress(jsonText, opts = {}) {
    let parsed;
    try {
    ...
    if (opts.dictionary) {
        ...
        if (col && (!best || col.length < best.length))
            best = col;
    }
    if (!best || best.length >= jsonText.length)
        return null;
    return best;
}
