...
function collapseDuplicates(lines) {
    const out = [];
    let i = 0;
    while (i < lines.length) {
        const line = lines[i] ?? "";
        let n = 1;
        while (i + n < lines.length && lines[i + n] === line)
            n++;
...
function renderFrameRun(lines, start, n) {
    if (n <= KEEP_FRAMES + 1)
        return lines.slice(start, start + n).map((l) => l ?? "");
...
function collapseStacks(lines) {
    ...
    while (i < lines.length) {
        ...
        let n = 0;
        while (i + n < lines.length && STACK_FRAME.test(lines[i + n] ?? ""))
            n++;
